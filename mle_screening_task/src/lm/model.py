...
class MultiHeadAttention(nn.Module):
    ...
    def self_attention(
        ...
        """
        s = qkT.size(-1)
        causal_mask = torch.tril(torch.ones(s, s, device=qkT.device))
        ...
        if attention_mask is None:
            ...
            mask = (causal_mask * attention_mask[:, :, None] * attention_mask[:, None])[:, None].bool()
        ...
        attn_logits = torch.masked_fill(unmasked_attn_logits, ~mask, float_min)
        #print(attn_logits)
        ...
        attn = rearrange(attn, 'b h s hd -> b s (h hd)').contiguous()
        #print(attn.shape)
...
class DecoderLM(nn.Module):
    ...
    def token_logits(self, x: torch.FloatTensor) -> torch.FloatTensor:
        ...
        return F.linear(self.ln(x), self.token_embeddings.weight)
    ...
    def forward(
        ...
        logits = self.token_logits(last_hidden_state) #self.embed(input_ids, attention_mask)) # ...
        return logits
